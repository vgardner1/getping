-- Drop and recreate get_public_profile_secure function to remove sensitive data
...
-- Create a function for getting contact info (only for the owner)
...
AS $$
  SELECT 
    p.phone_number,
    -- Get email from auth.users securely via the existing secure function
    (SELECT get_user_email_for_contact(target_user_id)) as email,
