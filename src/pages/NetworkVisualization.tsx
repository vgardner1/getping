...
export default function NetworkVisualization() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('event-attendance-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  const loadUserEvents = async () => {
    ...
    const {
      data: attendances,
      error
    } = await supabase.from('event_attendances').select('event_id, events(id, name)').eq('user_id', user.id).eq('status', 'going');
    if (error) {
  ...
  const loadEventAttendees = async () => {
    ...
    const {
      data: attendances,
      error
    } = await supabase.from('event_attendances').select('user_id, event_id').in('event_id', eventIds).eq('status', 'going').neq('user_id', user.id); // Exclude current user
    ...
    const {
      data: profiles,
      error: profilesError
    } = await supabase.from('profiles').select('user_id, display_name').in('user_id', attendeeUserIds);
    if (profilesError) {
    ...
    } = await supabase.from('connections').select('user_id, target_user_id').or(`user_id.eq.${user.id},target_user_id.eq.${user.id}`);
    if (connectionsError) {
      console.error('Error loading connections', connectionsError);
    }

    // Create a set of connected user IDs
    const connectedUserIds = new Set(connections?.map((c: any) => c.user_id === user.id ? c.target_user_id : c.user_id) || []);
  ...
  const loadRealConnections = async () => {
    ...
    const otherUserIds = Array.from(new Set(connections.map((c: any) => c.user_id === user.id ? c.target_user_id : c.user_id)));
    ...
    const {
      data: profiles,
      error: profilesError
    } = await supabase.from('profiles').select('user_id, display_name, location').in('user_id', otherUserIds);
    if (profilesError) {
  ...
  const loadDemoData = () => {
    const circles: Array<'family' | 'friends' | 'business' | 'acquaintances' | 'network' | 'extended'> = ['family', 'friends', 'business', 'acquaintances', 'network', 'extended'];
    ...
    circles.forEach((circle, circleIdx) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const healthScore = circleIdx === 0 ? 85 + Math.random() * 15 : circleIdx === 1 ? 75 + Math.random() * 20 : circleIdx === 2 ? 70 + Math.random() * 25 : circleIdx === 3 ? 60 + Math.random() * 30 : circleIdx === 4 ? 55 + Math.random() * 35 : 45 + Math.random() * 40;
  ...
  return <div className="relative min-h-screen bg-background overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-b from-black/80 via-black/40 to-transparent pb-2">
        <div className="flex items-center justify-between p-2 md:p-3">
          <Button variant="ghost" onClick={() => navigate('/network/visualize')} className="text-white hover:bg-white/10 h-7 px-3 md:h-9 md:px-4 font-bold text-xs md:text-sm">
            ping!
          </Button>
      ...
      <div className="absolute left-0 top-12 md:top-14 z-20">
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="outline" size="icon" className="rounded-r-lg rounded-l-none border-l-0 bg-black/80 backdrop-blur border-primary/30 hover:bg-primary/20 h-8 w-8">
      ...
      <div className="absolute right-4 bottom-24 md:bottom-28 z-20">
        <div className="bg-black/90 backdrop-blur border border-primary/30 rounded-lg p-3 w-48">
          <div className="flex items-center justify-between mb-2">
            ...
            <Button variant="ghost" size="icon" onClick={() => setIsDemoMode(!isDemoMode)} className="h-6 w-6">
          ...
          <Button variant={isDemoMode ? "default" : "outline"} onClick={() => setIsDemoMode(!isDemoMode)} className="w-full mt-2 h-7 text-xs">
      ...
      {/* Bottom Chats Drawer */}
      <div className={`fixed bottom-0 left-0 right-0 bg-black/95 backdrop-blur-xl border-t border-primary/30 shadow-2xl transition-all duration-300 z-40 ${chatDrawerState === 'hidden' ? 'translate-y-[calc(100%-3rem)]' : chatDrawerState === 'half' ? 'translate-y-[50%]' : 'translate-y-0'}`} style={{
      height: '80vh'
    }}>
        {/* Handle Bar */}
        <button onClick={handleChatDrawerToggle} className="absolute top-0 left-1/2 -translate-x-1/2 w-full p-3 flex flex-col items-center gap-1 hover:bg-white/5 transition-colors">
