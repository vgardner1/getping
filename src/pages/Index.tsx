      ...
      <main className="max-w-4xl mx-auto p-6 pb-28 relative z-10 safe-bottom">
        ...
        <Card className="bg-card border-border p-8 max-w-md mx-auto space-y-8">
          ...
          <div className="text-center space-y-6">
            ...
            <div className="space-y-4">
              <h1 className="text-4xl md:text-5xl font-bold">
                {words.map((word, index) => (
                  <span
                    key={index}
                    className={`inline-block mr-4 iridescent-text transition-all duration-700 transform ${index < visibleWords ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-95"}`}
                  >
                    {word.toLowerCase()}
                  </span>
                ))}
              ...
              <p
                className={`text-sm md:text-base text-muted-foreground iridescent-text transition-all duration-700 delay-1000 ${visibleWords >= words.length ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
              >
            ...
            <div
              className={`flex flex-col gap-3 transition-all duration-700 delay-1500 ${visibleWords >= words.length ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
            >
              <Link to="/signup">
              ...
              <Link to="/signin">
                <Button
                  variant="outline"
                  className="w-full shimmer border-primary text-primary hover:bg-primary/10 hover:scale-105 transition-transform duration-200"
                >
                  sign in
                </Button>
              </Link>
            ...
            <div
              className={`transition-all duration-700 delay-2000 ${visibleWords >= words.length ? "opacity-100" : "opacity-0"}`}
            >
              <Link
                to="/welcome"
